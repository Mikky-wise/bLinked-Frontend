  ...
  return (
    <div className="home-pending-order-grid-box py-4">
      ...
      <div className="d-flex justify-content-between align-items-center mt-4">
        ...
        <span className="home-pending-order-status px-3 py-1 rounded-pill" style={{
            background: item.status === "New" ? "#FFEBEC" : item.status === "Enroute Dropoff" ? "#E9EEFF" : "#FCF2E3",
            color: item.status === "New" ? "#FF4554" :  item.status === "Enroute Dropoff" ? "#1752FF" : "#F1872D",
        }}>{item.status}</span>
      </div>
    </div>
  );
